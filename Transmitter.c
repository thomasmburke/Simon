...
int main(void)
{
   ...
   srand(time(seed)); //initial random number seed
...
void init_UART()
{
   ...
   IFG2 &= ~UCA0RXIFG;
   UC0IE |= UCA0RXIE; //+ UCA0TXIE;   // Enable USCI_A0 transmit and receive interrupts
}
...
interrupt void WDT_interval_handler()
{
	...
	case 'M':
		...
		if (last_button2 && (d == 0) && multiplayer) //handle the button press
		{
			P1OUT &= ~MODE2;
			state = 'A';
		}
